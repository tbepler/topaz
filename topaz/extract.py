...
def extract_particles(paths:List[str], model:Union[torch.nn.Module, str], device:int, batch_size:int, threshold:float, radius:int, num_workers:int, targets:str, 
    ...
    if not only_validate:
        ...
        if not per_micrograph:
            ...
            output_dir = os.path.join(output, 'COORDS')
            ...
            if per_micrograph:
                ...
                if dims == 2:
                    ...
                    with open(out_path, 'w') as f:
                        file_utils.write_table(f, table, format=out_format)
                else:
                    imod_coords_path = os.path.join(output_dir, name + suffix + '.coords')
                    with open(imod_coords_path, 'w') as f:
                        for coord in coords:
