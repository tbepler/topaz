...
class MemoryMappedImage():
    ...
    def get_crop(self, center_indices):
        ...
        xpad = max(0, -xmin), max(0, xmax - self.shape[-1])
        ypad = max(0, -ymin), max(0, ymax - self.shape[-2])

        zmin, zmax, zpad = None, None, (0, 0)
        if z is not None:
            zmin = max(0, z - self.size // 2)
            zmax = min(self.shape[-3], z + self.size // 2 + 1)
            zpad = max(0, -zmin), max(0, zmax - self.shape[-3])

        # Create a copy of the data only when necessary
        if self.dims == 3:
            crop = self.mmap[zmin:zmax, ymin:ymax, xmin:xmax]
            if any(pad > 0 for pad in (*zpad, *ypad, *xpad)):
                crop = np.pad(crop, (zpad, ypad, xpad), mode='constant')
        else:
            crop = self.mmap[ymin:ymax, xmin:xmax]
