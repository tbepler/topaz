...
class CroppableImage:
    ...
    def __init__(self, image_path: str, targets: pd.DataFrame, crop_size: int,
                 split: str = 'pn', dims: int = 2, use_cuda: bool = False, mask_size: int = 123):
...
class MemoryMappedImage(CroppableImage):
    ...
    def __init__(self, image_path: str, targets: pd.DataFrame, crop_size: int,
        ...
        super().__init__(image_path, targets, crop_size, split, dims, use_cuda, mask_size)
...
class MultipleImageSetDataset(torch.utils.data.Dataset):
    def __init__(self, paths:List[List[str]], targets:pd.DataFrame, number_samples:int, crop_size:int, image_set_balance:List[float]=None, 
                 ...
                 use_cuda:bool=False, mask_size=123, preload:bool=True):
        ...
        for group in paths:
            ...
            for path in group:
                ...
                image = PreloadedImage(path, img_targets, crop_size, split, dims=dims, use_cuda=use_cuda, mask_size=mask_size) if preload \
                    else MemoryMappedImage(path, img_targets, crop_size, split, dims=dims, use_cuda=use_cuda, mask_size=mask_size) 
